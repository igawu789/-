import numpy as np
from normalization import normalize_noq2,denormalize,normalize,flatten_and_normalize
from io_utils import save_image
from rf import train_random_forest
import gc

def process_image_for_prediction(image_data, rf_model, src_min, src_max, target_min, target_max):
    ...
    # 上述只是为了要一个
    # 使用随机森林预测
    predictions_normalized = rf_model.predict(normalized_data)
...
def process_image_in_blocks(image_data, rf_model, src_min, src_max, target_min, target_max, block_size):
    ...
    # 归一化（使用 clip 限制范围）
    # normalized_data = np.clip((valid_data - src_min) / (src_max - src_min), 0, 1)
    normalized_data, src_min, src_max = normalize(valid_data)
    ...
    for i in range(0, h, block_size):
        for j in range(0, w, block_size):
            ...
            # 处理该块
            processed_block = process_image_for_prediction(block, rf_model, src_min, src_max, target_min, target_max)
...
def fuse_images(overlap1, overlap2, image1, image2, path1,path2,output_dir, block_size,rf_params,ui_callback):
    # 处理重叠区域数据获取归一化参数
    norm_overlap1, norm_overlap2, min_val1, max_val1, min_val2, max_val2 = flatten_and_normalize(overlap1, overlap2)

    # 打印形状和最值信息
    print(f"norm_overlap1 形状: {norm_overlap1.shape}, 最小值: {min_val1}, 最大值: {max_val1}")
    print(f"norm_overlap2 形状: {norm_overlap2.shape}, 最小值: {min_val2}, 最大值: {max_val2}")
    """norm_overlap1 形状: (33378393, 1), 最小值: 12.626163482666016, 最大值: 198.03005981445312
norm_overlap2 形状: (33378393, 1), 最小值: 11.325870513916016, 最大值: 108.14346313476562"""

    # 打印形状、最小值和最大值信息
    print(
        f"norm_overlap1 形状: {norm_overlap1.shape}, 最小值: {np.min(norm_overlap1)}, 最大值: {np.max(norm_overlap1)}")
    print(
        f"norm_overlap2 形状: {norm_overlap2.shape}, 最小值: {np.min(norm_overlap2)}, 最大值: {np.max(norm_overlap2)}")


    # 训练模型
    rf_A_to_B = train_random_forest(norm_overlap1, norm_overlap2, rf_params)  # Pass params
    rf_B_to_A = train_random_forest(norm_overlap2, norm_overlap1, rf_params)  # Pass params


    # 处理完整图像
    print("\nProcessing full image A->B...")
    # **使用分块方式处理完整图像**
    print("\nProcessing full image A->B with blocks...")
    fused_A_to_B = process_image_in_blocks(
        image1, rf_A_to_B,
    ...
    print("\nProcessing full image B->A with blocks...")
    fused_B_to_A = process_image_in_blocks(
        image2, rf_B_to_A,
        src_min=min_val2, src_max=max_val2,
        target_min=min_val1, target_max=max_val1,
        block_size=block_size
    )

    save_image(f"{output_dir}/LocalB_2.tif", fused_A_to_B, path2)
    save_image(f"{output_dir}/LocalA_2.tif", fused_B_to_A, path1)

    return fused_A_to_B, fused_B_to_A
